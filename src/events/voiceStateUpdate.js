...
function endSession(guildId, userId) {
  ...
  if (session.eligible) {
    ...
    if (chunks > 0) {
      addPoints(userId, session.guildId, applyBuff(userId, session.guildId, 'points', chunks * VOICE.POINTS_PER_INTERVAL));
      addXp(userId, session.guildId, applyBuff(userId, session.guildId, 'xp', chunks * VOICE.XP_PER_INTERVAL));
    }
  }
...
function syncEligibility(guildId, userId) {
  ...
  if (!eligible) {
    const chunks = Math.floor((now - session.lastGrant) / VOICE.INTERVAL_MS);
    if (chunks > 0) {
      addPoints(session.userId, guildId, applyBuff(session.userId, guildId, 'points', chunks * VOICE.POINTS_PER_INTERVAL));
      addXp(session.userId, guildId, applyBuff(session.userId, guildId, 'xp', chunks * VOICE.XP_PER_INTERVAL));
    }
  }
...
setInterval(() => {
  ...
  for (const session of sessions.values()) {
    ...
    addXp(session.userId, session.guildId, applyBuff(session.userId, session.guildId, 'xp', VOICE.XP_PER_INTERVAL));
    session.lastGrant = now;
    saveVoiceSession(session.userId, session.guildId, session);
  }
}, VOICE.INTERVAL_MS);
