...
function getActiveBoss(guildId) {
  return db.prepare(
    "SELECT * FROM boss_spawns WHERE guildId = ? AND status = 'active' ORDER BY id DESC LIMIT 1",
  ).get(guildId);
}
...
function getContributions(bossId) {
  return db.prepare('SELECT userId, damage, hits FROM boss_damage WHERE bossId = ? ORDER BY damage DESC')
    .all(bossId);
}
...
function applyDamage(bossId, userId, damage, now = Date.now()) {
  ...
  const run = db.transaction(() => {
    db.prepare('UPDATE boss_spawns SET hp = ?, lastHitUserId = ? WHERE id = ?')
      .run(hpLeft, hpLeft <= 0 ? userId : row.lastHitUserId, bossId);
    db.prepare(`
...
function expireBoss(bossId, now = Date.now()) {
  db.prepare("UPDATE boss_spawns SET status = 'escaped', endedAt = ? WHERE id = ? AND status = 'active'")
    .run(now, bossId);
}
...
function distributeRewards(bossId, rng = Math.random) {
  ...
  for (const entry of rewards) {
    ...
    const lootRate = getMultiplier(userId, guildId, 'boss_loot_rate') * getDebuff(userId, guildId, 'debuff:loot');
