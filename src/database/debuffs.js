...
const { DEBUFF_PREFIX, pickDebuff, isDebuffRow, stealAmount, getBossAttack } = require('../lib/bossAttacks');
...
function getActiveDebuffs(userId, guildId, now = Date.now()) {
  return db
    ...
    .filter(
      row => (row.expiresAt == null || row.expiresAt > now) && (row.charges == null || row.charges > 0),
    );
}
