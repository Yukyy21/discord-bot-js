  ...
  async execute(interaction) {
    ...
    if (subcommand === 'deposit' && before.balance < amount) {
      return interaction.reply({
        embeds: [
          errorEmbed(`Dompet tidak cukup. Isinya **${before.balance.toLocaleString()}** ${e('coin')}.`),
        ],
    ...
    if (subcommand === 'withdraw' && before.bank < amount) {
      return interaction.reply({
        embeds: [
          errorEmbed(`Isi bank tidak cukup. Ada **${before.bank.toLocaleString()}** ${e('coin')} di bank.`),
        ],
        flags: MessageFlags.Ephemeral,
      });
    }
