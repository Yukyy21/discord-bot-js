...
async function handleBossAttack(interaction, bossId) {
  ...
  if (left > 0) {
    return interaction.reply({
      embeds: [warnEmbed(`${e('clock')} Senjatamu masih dingin. Coba lagi <t:${Math.floor((Date.now() + left) / 1000)}:R>.`)],
      flags: MessageFlags.Ephemeral,
    });
  }
...
async function escapeBoss(client, row) {
  ...
  await channel
    .send({ embeds: [bossEscapedEmbed(row)], files: bossIconFiles(row.bossKey) })
    .catch(() => {});
