...
const TYPES = ['chat', 'voice', 'daily', 'use_item', 'buy_item', 'spend', 'give', 'level_up', 'use_tier', 'daily_streak', 'boss_join', 'boss_kill'];
...
test('currentPeriodKeys memuat ketiga scope', () => {
  ...
  for (const scope of SCOPES) {
    assert.ok(keys.some(k => k.startsWith(`${scope}:`)), `scope ${scope} hilang`);
  }
});
...
test('quest mode max (streak) dan meta (tier) ada di katalog', () => {
  assert.ok(Object.values(QUEST_CATALOG).some(q => q.mode === 'max'), 'tidak ada quest ber-mode max');
  assert.ok(Object.values(QUEST_CATALOG).some(q => q.meta), 'tidak ada quest ber-meta');
});
