...
test('tiap boss punya daftar serangan valid dan peluang balas', () => {
  ...
  assert.strictEqual(rollCounter(BOSS_CATALOG.pump_freakin, () => 0.99), false);
  assert.strictEqual(rollCounter(BOSS_CATALOG.ancient_mummy, () => 0.01), true);
});
...
test('amukan boss hanya menyasar penyerang yang masih hangat', () => {
  ...
  assert.deepStrictEqual(targets.map(t => t.userId), ['b', 'a', 'd']);
  assert.strictEqual(targets.length, BOSS.RAMPAGE_TARGETS);
});

test('deskripsi debuff terbaca manusia', () => {
  const now = 0;
  assert.match(describeDebuff({ key: 'debuff:cooldown', value: 2, expiresAt: 5 * 60000, charges: null }, now), /Cooldown serang ×2/);
  assert.match(describeDebuff({ key: 'debuff:miss', value: 1, expiresAt: null, charges: 1 }, now), /1x serangan/);
});
