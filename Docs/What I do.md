...
## Gelombang Sembilan Belas — `/beta` & `/report`

Dua command baru dari owner: `/beta type:on|off` (admin) buat pengingat
mode beta, dan `/report type:bug|saran text:` (semua member) buat lapor
bug/kasih saran langsung ke DM admin.

Desain pengingat beta sempat direvisi di tengah sesi ini: iterasi pertama
broadcast ke satu channel tetap tiap 45-90 menit (mirip `statusRotator.js`).
Owner minta ubah — pengingat harus numpang ephemeral di command/tombol
apa pun yang sedang dipakai user saat itu (misal lagi buka `/shop`), bukan
pesan terpisah, dengan cooldown 40-90 menit **per user** (bukan per guild).
`src/lib/betaAnnouncer.js` (broadcaster lama) dihapus total, diganti
`src/lib/betaReminder.js` dengan mekanisme di bawah.

### `/beta type:on|off` — admin, per-guild (switch saja, tidak berubah)

- `src/database/schema.js`: tabel `guild_config` dapat kolom baru
  `betaMode INTEGER DEFAULT 0` — ditambahkan baik ke `CREATE TABLE` (install
  baru) maupun lewat `ensureColumn()` di `runMigrations()` (database lama
  otomatis ketambahan kolom ini tanpa perlu reset/hapus DB).
- `src/database/guildConfig.js`: `getBetaMode(guildId)`,
  `setBetaMode(guildId, enabled)`. Pola query sama persis dengan
  `setBossChannel`/`clearBossChannel` yang sudah ada (`INSERT ... ON
  CONFLICT DO UPDATE`). (`getAllBetaGuilds()` dari iterasi broadcast
  pertama sudah tidak dipakai — desain baru tidak lagi mem-poll semua
  guild, tapi fungsinya dibiarkan ada di `guildConfig.js` kalau-kalau
  berguna untuk command admin lain nanti, mis. `/beta status` global.)
- `src/commands/admin/beta.js` (baru): `setDefaultMemberPermissions(Administrator)`,
  sama seperti `/boss-channel`. Kalau state yang diminta sama dengan state
  sekarang (mis. `/beta on` padahal sudah on), tidak nulis ulang ke DB —
  cuma kasih tahu "memang sudah nyala".

### Pengingat beta — numpang ephemeral, cooldown per user (desain final)

- `src/database/schema.js`: tabel baru `beta_reminders` (`userId, guildId,
  nextEligibleAt`, PK gabungan) — melacak kapan tiap user boleh dikirimi
  pengingat berikutnya. Ini terpisah dari `guild_config.betaMode` (itu
  saklar on/off per server, ini cooldown per orang).
- `src/database/betaReminders.js` (baru): `isBetaReminderDue(userId,
  guildId)` (true kalau belum pernah atau cooldown sudah lewat),
  `markBetaReminderSent(userId, guildId, nextEligibleAt)`.
- `src/config/constants.js`: objek `BETA` — `MIN_INTERVAL_MS`/`MAX_INTERVAL_MS`
  diubah dari rencana awal 45-90 menit menjadi **40-90 menit** sesuai angka
  yang diminta owner.
- `src/lib/betaReminder.js` (baru, gantikan `betaAnnouncer.js`):
  `maybeSendBetaReminder(interaction)`. Alur: kalau `guildId` kosong (DM)
  → skip; kalau interaksi belum dibalas/di-defer sama sekali → skip (jaga-
  jaga, `followUp` butuh itu); kalau `betaMode` guild mati → skip; kalau
  `isBetaReminderDue` bilang belum waktunya → skip. Kalau semua syarat
  lolos: **kunci cooldown dulu** (`markBetaReminderSent` dengan jeda acak
  baru) baru `interaction.followUp({ content: ..., flags: Ephemeral })` —
  urutan ini sengaja supaya kalau `followUp` gagal (mis. token interaksi
  sudah lewat window Discord), yang terjadi adalah "user kelewat satu
  giliran pengingat", bukan bot mencoba kirim ulang terus-menerus di
  command berikutnya.
- `src/events/interactionCreate.js`: dispatcher utama dirombak dari
  `if/return` berantai jadi `if/else if` supaya ada satu titik setelah
  ketiganya (`handleButton`/`handleSelectMenu`/`handleCommand`) selesai —
  di situ `maybeSendBetaReminder(interaction)` dipanggil untuk **semua**
  jenis interaksi (command, tombol, dropdown), bukan cuma slash command,
  supaya "lagi asik pencet tombol next `/shop`" juga terhitung sebagai
  aktivitas yang layak diselipi pengingat. Dikecualikan dua customId:
  `guide_close` dan `pager_noop` — keduanya bukan aksi substantif, dan
  memunculkan pengingat pas user baru saja menutup sesuatu terasa janggal.
  Kegagalan `maybeSendBetaReminder` di-catch dan cuma di-log, tidak pernah
  melempar balik ke pemanggil (tidak boleh mengganggu command asli yang
  sudah sukses).
- `src/events/ready.js`: pemanggilan `startBetaAnnouncer(client)` dihapus
  lagi (desain baru tidak butuh scheduler `setTimeout` terpisah — semuanya
  reaktif, dipicu oleh interaksi user, bukan jadwal berjalan sendiri).
- `src/lib/levelingManager.js`: `resolvePointChannel` yang sempat diekspor
  untuk kebutuhan broadcaster lama **dibiarkan tetap diekspor** (tidak
  merugikan, dan levelingManager.js sendiri masih memakainya secara
  internal) walau sekarang tidak lagi dipakai fitur beta.

### `/report type:bug|saran text:<isi>` — semua member (tidak berubah dari rencana awal)

- `src/commands/general/report.js` (baru): dua opsi wajib, `type` (choices
  `bug`/`saran`) dan `text` (`setMaxLength(1000)`). Balasan **ephemeral** —
  cuma pelapor yang lihat konfirmasinya.
- **Tidak ada tabel baru buat isi laporan.** Laporan tidak disimpan —
  langsung diteruskan lewat **DM** ke admin saat itu juga, sesuai
  permintaan eksplisit owner ("masuk di DM admin").
- `notifyAdmins()` di file yang sama **mereplikasi pola** yang sudah ada di
  `notifyAdmins()` milik `src/commands/economy/poruvShop.js` (bukan
  diekstrak ke helper bersama — proyek ini belum punya lib khusus
  notifikasi admin, jadi duplikasi kecil ini konsisten dengan gaya yang
  sudah ada): kumpulkan member unik dari semua role di `ADMIN_ROLE_IDS`,
  DM paralel lewat `Promise.allSettled`, kegagalan per-DM tidak
  menggagalkan DM ke admin lain atau menggagalkan laporan yang sudah
  diterima usernya.
- **`ADMIN_ROLE_IDS` dipakai bersama** dengan `/poruv-shop` — bukan env
  baru. `.env.example` diperbarui komentarnya supaya jelas dipakai dua
  fitur.
- Embed laporan menyertakan pelapor (mention + user ID) dan nama server —
  admin tidak perlu tanya balik "ini dari server mana, siapa yang lapor".

### File yang disentuh
`src/database/schema.js`, `src/database/guildConfig.js`,
`src/database/betaReminders.js` (baru), `src/database/index.js`,
`src/config/constants.js`, `src/lib/betaReminder.js` (baru, gantikan
`betaAnnouncer.js` yang dihapus), `src/events/ready.js`,
`src/events/interactionCreate.js`, `src/commands/admin/beta.js` (baru),
`src/commands/general/report.js` (baru), `.env.example`.

### Belum sempat diverifikasi otomatis
Sandbox sesi ini tidak punya akses `npm install`, jadi verifikasi terbatas
pada `node --check` per file dan pengecekan silang manual (export/import
cocok, tidak ada nama fungsi bentrok, tidak ada sisa referensi ke
`betaAnnouncer.js` yang sudah dihapus). **`npm test` dan `npm run lint`
belum dijalankan** — perubahan di `interactionCreate.js` menyentuh
dispatcher utama semua interaksi (button/select/command), jadi walau
perubahannya aditif (menambah satu pemanggilan di akhir, bukan mengubah
logic yang sudah ada), ini bagian yang paling penting dites sebelum merge.

## Gelombang Delapan Belas — Asset Boss, Credit, Guide Ephemeral, & Auto-Attack

Empat permintaan owner dalam satu sesi: rapikan `assets/boss/` yang berantakan,
hapus satu orang dari `/credit`, `/guide` jadi ephemeral, dan rombak total alur
serang boss (tombol, pesan, dan amukan) supaya tidak menuh-menuhin channel.

### Asset boss dibereskan

- `assets/boss/assets/boss/pump_freakin.png` — folder nested tidak sengaja
  (`assets/boss/assets/boss/`, harusnya cuma `assets/boss/`). Akibatnya
  `asset('boss', 'pump_freakin.png')` di `src/lib/paths.js` tidak pernah
  ketemu filenya, icon Pump Freakin selalu gagal dimuat (fallback diam-diam:
  embed tetap terkirim tanpa gambar, tidak error). File dipindah balik ke
  `assets/boss/pump_freakin.png`, folder nested dihapus. Ini murni salah taruh
  folder, bukan masalah konten gambarnya.
- `clown_orca.png` di `src/lib/bossCatalog.js` menunjuk ke file yang memang
  belum pernah ada — filenya masih bernama hash upload asli
  (`2f62ece473080be6ec0384f6052c7a7b.jpg`), belum sempat di-rename. Di-rename
  jadi `assets/boss/clown_orca.jpg`, dan `bossCatalog.js` diperbaiki
  nunjuk ke `clown_orca.jpg` (bukan `.png` — ekstensi asli file JPG).

Ketiga boss (Pump Freakin, Clown Orca, Ancient Mummy) sekarang punya asset
yang benar-benar termuat.

### `/credit` — satu kontributor dicopot dari Boss Artwork

- `src/commands/general/credit.js`: user `1537045204169924688` dihapus dari
  grup **Boss Artwork** (array `members` dikosongkan), tapi **tetap** ada di
  grup **Beta Tester** — permintaan eksplisit owner, bukan hapus total.
- `buildCredit()` di file yang sama: `.addFields()` sekarang `.filter(g =>
  g.members.length > 0)` sebelum di-map. Tanpa filter ini, grup dengan
  `members: []` menghasilkan field embed dengan `value: ''`, yang ditolak API
  Discord (field embed wajib 1-1024 karakter). Perbaikan ini berlaku umum —
  grup manapun di masa depan yang sempat kosong tidak akan menjatuhkan
  seluruh command.

### `/guide` jadi ephemeral

- `src/commands/general/guide.js`: balasan awal `interaction.reply()`
  ditambah `flags: MessageFlags.Ephemeral` — cuma pemanggil yang lihat,
  member lain di channel tidak lagi kebanjiran pesan guide orang lain.
- Navigasi dropdown (`guide_select`) dan tombol kembali ke beranda
  (`guide_home`) di `src/events/interactionCreate.js` tidak perlu diubah:
  keduanya pakai `interaction.update()`, yang otomatis mempertahankan flag
  ephemeral dari pesan asal.
- Tombol **Tutup** (`guide_close`) harus diubah: sebelumnya
  `interaction.message.delete()`, yang **ditolak API Discord** untuk pesan
  ephemeral (bot tidak bisa hapus ephemeral lewat method itu — gagal senyap
  karena dibungkus `.catch(() => {})`, tombol Tutup jadi tidak berfungsi).
  Diganti `interaction.deferUpdate().then(() => interaction.deleteReply())`,
  yang valid untuk ephemeral karena lewat webhook interaksi, bukan REST pesan
  biasa.

### Tombol Serang! → Toggle Auto Attack (maks 20×)

Permintaan owner: user tidak perlu klik berkali-kali tiap 10 detik.

- `src/ui/bossEmbeds.js`: `attackRow()` — tombol diganti dari `boss_attack:
  <id>` label "Serang!" menjadi `boss_autoattack:<id>` label **"Toggle Auto
  Attack"**.
- `src/lib/bossManager.js`: logic serangan dipecah jadi dua lapis.
  - `performAttack(client, row, userId, guildId)` — fungsi murni satu
    serangan (cooldown check, roll damage, buff/debuff, counter-attack, quest
    progress, update embed boss publik). Tidak menyentuh objek `interaction`
    sama sekali, jadi bisa dipanggil baik dari klik manual maupun dari loop
    auto-attack tanpa duplikasi logic.
  - `handleBossAutoAttack(interaction, bossId)` — handler tombol baru.
    Klik pertama: `deferReply` ephemeral, langsung serang, lalu dijadwalkan
    ulang tiap `BOSS.ATTACK_COOLDOWN_MS` (menyesuaikan kalau kena debuff
    cooldown) sampai **20 kali serangan** (`AUTO_ATTACK_MAX`) atau boss
    tumbang duluan. Klik tombol lagi selagi loop jalan = berhenti lebih awal.
    Setelah mentok 20× atau berhenti, user harus klik tombolnya lagi untuk
    memulai batch baru — sesuai permintaan (bukan otomatis selamanya).
  - State loop disimpan di memori proses (`Map` `bossId:userId` → timer),
    bukan database — cukup untuk kebutuhan ini karena hanya perlu bertahan
    selama bot menyala; restart bot otomatis menghentikan loop yang sedang
    jalan (user tinggal klik toggle lagi).
- `src/events/interactionCreate.js`: customId `boss_attack` → `boss_autoattack`,
  memanggil `handleBossAutoAttack` (bukan lagi `handleBossAttack`, yang sudah
  dihapus total).

### Satu pesan, di-edit terus — bukan spam pesan baru

Permintaan owner: hasil serang boss & counter-attack cuma **satu pesan**
yang di-edit berulang, bukan pesan baru tiap serangan (auto-attack bisa
sampai 20× — kalau tiap kali kirim baru, ephemeral channel-nya sendiri
langsung penuh untuk user itu).

- `handleBossAutoAttack` memakai `interaction.editReply()` di setiap
  iterasi loop, bukan `interaction.followUp()`. Pesan ephemeral pertama
  (dari `deferReply`) itulah yang terus di-edit sampai loop selesai.
- `attackResultEmbed()` di `src/ui/bossEmbeds.js` dapat parameter baru
  `autoAttack: { count, max, done, cooldownSec }` — menampilkan progres
  "Auto attack 7/20" di baris terakhir embed, plus pesan "klik lagi buat
  lanjut" begitu batch selesai.

### Gambar boss dicabut dari pesan serang & counter-attack (kebesaran)

Permintaan owner: gambar boss di `attackResultEmbed` (hasil serang milik
sendiri, termasuk counter-attack balik boss yang nempel di embed yang sama)
dianggap kebesaran untuk konteks itu.

- `attackResultEmbed()`: `.setImage(icon.url)` dan parameter `files` dihapus
  total dari fungsi ini dan dari pemanggilnya di `performAttack()`.
- Gambar boss **tetap** tampil di tempat lain yang memang butuh, tidak
  disentuh: embed spawn (`bossEmbed`), embed boss tumbang
  (`bossDefeatedEmbed`), dan embed boss kabur (`bossEscapedEmbed`).

### Amukan boss (rampage) — dari 1 embed publik jadi DM per orang

Permintaan owner: pesan serangan boss ke player (baik lewat klik sendiri
maupun amukan berkala) jangan menumpuk di channel boss, dan sebisa mungkin
cuma penerimanya yang lihat.

- Counter-attack dari klik sendiri sudah ephemeral sejak sebelumnya (nempel
  di `attackResultEmbed`, lihat bagian di atas) — bagian yang berubah di
  gelombang ini adalah **amukan berkala** (`rampageBoss()`), yang sebelumnya
  satu embed gabungan nge-tag beberapa top-attacker sekaligus di channel
  boss, tiap `BOSS.RAMPAGE_INTERVAL_MS`.
- **Catatan teknis penting**: flag `ephemeral` Discord murni cuma berlaku
  sebagai balasan sebuah interaksi (klik tombol/slash command) — amukan
  dipicu scheduler (`setInterval`), bukan interaksi user, jadi secara teknis
  tidak mungkin dibuat ephemeral beneran. Yang dipakai sebagai gantinya: DM
  langsung ke tiap user yang kena, efeknya sama (cuma penerima yang lihat,
  channel boss tidak kena spam sama sekali).
- `src/lib/bossManager.js`: `rampageBoss()` dirombak — loop `hits` sekarang
  `client.users.fetch(hit.userId)` lalu `.send()` DM satu-satu lewat
  `Promise.allSettled` (paralel, satu DM gagal tidak menggagalkan DM ke user
  lain). Tidak ada fallback ke channel publik kalau DM tertutup — sesuai
  permintaan, pesan itu dilewati saja (dicatat lewat `log.warn`).
- `src/ui/bossEmbeds.js`: `bossRampageEmbed()` (satu embed berisi banyak
  field, satu per user yang kena) diganti `bossRampageHitEmbed(row, hit)` —
  embed tunggal untuk satu user, tanpa mention (tidak perlu, ini DM) dan
  tanpa gambar boss (kebesaran, sama seperti `attackResultEmbed`).

### File yang disentuh
`src/commands/general/guide.js`, `src/commands/general/credit.js`,
`src/ui/bossEmbeds.js`, `src/lib/bossManager.js`,
`src/events/interactionCreate.js`, plus pemindahan/rename di `assets/boss/`.

### Belum sempat diverifikasi otomatis
Sesi ini tidak punya akses `npm install` (registry diblok jaringan sandbox),
jadi verifikasi terbatas pada `node --check` (syntax valid) untuk tiap file
yang diubah dan pengecekan silang manual ke seluruh referensi lama
(`handleBossAttack`, `boss_attack:`, `bossRampageEmbed` — dipastikan nol sisa
lewat `grep`). **`npm test` dan `npm run lint` belum dijalankan** — tolong
jalankan keduanya sebelum merge, terutama karena `bossManager.js` banyak
dirombak strukturnya (walau `test/boss.test.js` dan `test/bossAttacks.test.js`
tidak meng-import apapun dari `bossManager.js`, jadi kemungkinan besar tetap
hijau).
